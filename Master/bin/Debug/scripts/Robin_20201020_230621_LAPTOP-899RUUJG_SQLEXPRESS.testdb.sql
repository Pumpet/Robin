-- 20.10.2020 23:06:21 LAPTOP-899RUUJG\SQLEXPRESS.testdb 
-- Script for 'Robin'
...
select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = null)
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('ddDocs', 'Robin', @parentId, 1, 'Документы', 'down', 1, null, null)

select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = null)
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('ddDoc', 'Robin', @parentId, 1, 'Тест', '', 1, null, null)

select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = null)
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('miSimple', 'Robin', @parentId, 1, 'Simple Form', '', 0, '', null)

select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = 'ddDocs')
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('miDocList1', 'Robin', @parentId, 1, 'Документы-1', 'tmp', 0, 'DocList1', null)

select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = 'ddDoc')
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('ddTest1', 'Robin', @parentId, 1, 'Тест 1', '', 1, null, null)

select @parentId = (select id from dm.tMenu where appcode = 'Robin' and code = 'ddTest1')
insert dm.tMenu (code, appcode, parentId, ord, caption, img, execType, command, marker) 
    values ('miTest11', 'Robin', @parentId, 1, 'Тест метод', '', 0, 'TestMethod', null)
